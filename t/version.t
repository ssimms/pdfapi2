...
$pdf->header_version('1.3');
$pdf->trailer_version('1.5');

is($pdf->version(), '1.5',
   q{version() returns whichever version is largest (1/2)});

$pdf->header_version('1.6');
...
is($pdf->header_version(), '1.7',
   q{version() sets header version});

is($pdf->trailer_version(), '1.7',
...
$pdf->header_version('1.3');
$pdf->trailer_version('1.4');

my $version = $pdf->require_version('1.3');

is($version, '1.4',
   q{require_version returns current version});

$pdf->require_version('1.4');

is($pdf->header_version(), '1.3',
   q{require_version doesn't increase header version if trailer is sufficient});

$version = $pdf->require_version('1.5');
